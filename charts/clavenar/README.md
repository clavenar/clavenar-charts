...
helm pull oci://ghcr.io/clavenar/charts/clavenar \
  --version 0.38.5 --untar
curl -fsSLO \
  https://github.com/clavenar/clavenar-charts/releases/download/v0.38.5/clavenar-images-1.249.5.yaml
helm install my-clavenar ./clavenar \
  --namespace clavenar --create-namespace \
  --wait --wait-for-jobs --timeout 10m \
  -f ./clavenar/examples/values-bundled.yaml \
  -f ./clavenar-images-1.249.5.yaml
```

Every PVC created directly by the chart is annotated with
`helm.sh/resource-policy: keep`; StatefulSet claim templates retain their PVCs
under Kubernetes' default retention policy. `helm uninstall` therefore leaves
persistent data and the namespace in place. Use the public Clavenar uninstall
wrapper when you also want exact release-ownership checks or explicitly
...
The immutable, content-addressed `residual-product-disposition` ConfigMap
packages the exact `clavenar.residual-product-disposition/v1` schema and
fixture. Content addressing lets Helm replace a reviewed contract revision
without attempting Kubernetes' forbidden in-place update of immutable data. It
fixes all nine remaining product claims at one shipped static sandbox-annotation
corpus and eight explicitly deferred capabilities. The chart does not enable a
deferred capability: global notification routing is not tenant-selectable
routing, quorum is not named-group authorization, and a selectable WebAuthn
auth mode is not per-decision OIDC step-up.
