# Lab — Claude Code agent pod through clavenar-proxy

Optional scaffolding to drop a real **Claude Code CLI** into the same
Kubernetes cluster as the clavenar helm release, with every MCP tool
call routed through clavenar-proxy. End-to-end demo of the security
...
Edit tools are denylisted. Agent state lives in **clavenar-exec's**
...
```bash
kubectl -n clavenar logs deploy/<release>-exec --tail=200 | grep tools/call
kubectl -n clavenar exec deploy/<release>-exec -- ls /workspace
```

To run without the gateway (raw Claude Code + clavenar as one MCP
...
1. **A clavenar release with the bundled upstream stub.** The chart must
   ...
   call. Verify with:
   ```bash
   kubectl -n clavenar get pods | grep upstream-stub
   kubectl -n clavenar get jobs | grep vault-agent-seed
   ```
...
2. **An Anthropic API key Secret.** The agent pod reads
   ...
   ```bash
   kubectl -n clavenar create secret generic claude-code-agent-anthropic \
...
DNS:proxy.clavenar.local, DNS:localhost` (no `<release>-proxy` entry),
...
kubectl -n clavenar apply -f lab/manifests/mcp-config-cm.yaml
# Only when exec.enabled=true on the release. Skip otherwise.
kubectl -n clavenar apply -f lab/manifests/claude-code-managed-settings-cm.yaml
kubectl -n clavenar apply -f lab/manifests/agent-pod.yaml

kubectl -n clavenar wait --for=condition=Ready pod/claude-code-agent --timeout=120s
```
...
```bash
# Confirm Claude Code sees the clavenar MCP server.
kubectl -n clavenar exec -it claude-code-agent -- claude mcp list
# Expect: `clavenar` listed as an active server.

# Interactive session — fire any tool. The upstream-stub echoes back,
# but the request still hits Brain + Policy + ledger.
kubectl -n clavenar exec -it claude-code-agent -- claude

# Inspect the ledger row (port-forward in a second terminal).
kubectl -n clavenar port-forward svc/<release>-ledger 8083:8083 &
...
CN — see `repos/clavenar-ctl/docs/clients/claude-code.md` for the
...
```bash
kubectl -n clavenar delete -f lab/manifests/agent-pod.yaml
kubectl -n clavenar delete -f lab/manifests/mcp-config-cm.yaml
kubectl -n clavenar delete -f lab/manifests/claude-code-managed-settings-cm.yaml --ignore-not-found
kubectl -n clavenar delete secret claude-code-agent-anthropic
```

The chart itself is untouched — `helm uninstall <release>` cleans the
clavenar services + the bundled upstream-stub + the vault seed Job.

## Limitations

- **Shared identity.** Every pod uses the auto-mint starter cert
  (CN `agent-001`). The ledger conflates runs from multiple lab pods.
  Per-pod SVIDs (via `clavenar-identity` `POST /agents` + `/svid`) are a
  follow-up.
